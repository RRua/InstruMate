...
test_fnc() {
  ...
  local dest_dir=$4
  local dataset_dir=$5
  echo "G1 checking from ${min} to ${max} with tag ${tag} saving to ${dest_dir} using dataset ${dataset_dir}"
  if [ "$min" -gt "$max" ]; then
  ...
  for ((i = min; i <= max; i++)); do
    ...
    ./venv/bin/python start.py -m healthcheck -AS "${dataset_dir}" -o "${dest_dir}/dataset-${tag}-${i}" --recycle_emulator_with_kill --force --hc_capture_failed_apps  2>&1 | tee "/tmp/log-${tag}-${i}.log"
  done
}
