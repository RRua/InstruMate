...
class RepackagerByDroidFax(DeprecatedGenericRepackager):

    def __init__(self, name=None, tag='by-droidfax'):
        if name is None:
    ...
    def can_make(self, app: App, variant: AppVariant):
        ...
        if not knows_features:
            return False
        if not variant.is_feature_set(av.FEATURE_MONITOR_METHOD_CALLS_WITH_DROIDFAX):
            return False
        return True
    ...
    def modify_single_apk(self, unpacked_dir, is_base: bool, context: MakerContext):
        ...
        if context.merged_apk is not None:
            self.execute_tool(
                ToolDroidfaxInstrumentation(droidfax_dir=self.tool_droidfax, input_file=context.merged_apk,
            ...
            if context.input_app.has_split_pkgs():
                ...
                self.execute_tool(
                    ToolDroidfaxInstrumentation(droidfax_dir=self.tool_droidfax, input_file=input_app,
        ...
        context.context_data["droidfax"] = created_app.pop()

    def repack_single_apk(self, unpack_dir_path, is_base: bool, context: MakerContext):
        droidfax_file = context.context_data["droidfax"]
        if not os.path.exists(droidfax_file):
            raise RuntimeError("Missing droidfax instrumented file")
